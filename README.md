...
## Demo link
Add link here: `[DEMO LINK](https://olesmarola.github.io/react_people-table/)`
...
3. Implement `PersonRow` component accepting a `person` and displaying all the data described above
    ```html
    <tr class="PersonRow">
      <td></td>
      ...
    ...
    ```
    - add class `PersonRow--male`/`PersonRow--female` based on `sex`
...
5. add `century` column (`Math.ceil(person.died / 100)`)
    - add class `PersonRow--lived-in-${century}` to each `<tr>`

6. Mark a person row as selected when the user clicks on it (`PersonRow--selected`)
